...
session_start();
	...
	$sepet=$_SESSION["tablo"];
	echo $sepet;
	$total=0;
	$sql3=mysqli_query($col,"select * from ".$sepet);
	...
	if(@$_POST["ekle"]=="Ekle")
	{
		...
		{
			if($kayit4["urun_id"]==$_POST["urun_id"]){
				$kont=true;
		...
		if($kont==true){
			...
			@$duzsep=mysqli_query($col,"update $sepet set adet='$yeni_adet',toplam='$yeni_toplam'
				where urun_id='$guncelle_id'");
			if($duzsep) {
			...
			$sep=mysqli_query($col,"insert into $sepet (urun_id,urun_adi,adet,birim_fiyat,toplam) values($urun_id,'$urun_adi',$adet,$birim_fiyati,$toplam)");
			if($sep)
			{
				echo "Kayıt Eklendi";
				header("Location: #");
